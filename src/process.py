    ...
    with open(path, 'r', encoding='UTF-8') as f:
        for line in tqdm(f):
            ...
            if not lin:
                continue
            lin = lin.split('\t')
            seq1 = tokenizer(lin[0])
            seq2 = tokenizer(lin[1])
            seq1 = [x for x in seq1 if len(x) != 0 and x != ' ']
            seq2 = [x for x in seq2 if len(x) != 0 and x != ' ']
            if len(lin) != 3:
                contents.append([' '.join(seq1), ' '.join(seq2), -1000])
            else:
                contents.append([' '.join(seq1), ' '.join(seq2), int(lin[2])])
    return contents
...
def show_plt(data, config):
    ...
    data['text_len'] = data.apply(lambda x: len(x['seq1'].split()) + len(x['seq2'].split()), axis=1)
    ...
    data['text_len'] = data.apply(lambda x: len(''.join(x['seq1'].split())) + len(''.join(x['seq2'].split())), axis=1)
    print(data['text_len'].describe())
    plt.hist(data['text_len'], bins=200)
    plt.xlabel('sentence-length')
    plt.ylabel('category-number')
...
def main(config):
    ...
    train = load_dataset(config.data_dir + '/train.tsv')
    dev = load_dataset(config.data_dir + '/dev.tsv')
    test = load_dataset(config.data_dir + '/test.tsv')
